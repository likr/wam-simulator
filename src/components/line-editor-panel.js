...
const Slider = ({ property, value, digits, lineIndex, dispatch, children }) => {
  return (
    <div className='field'>
      <label className='label is-small has-text-centered'>
        {children} = {value.toFixed(2)}e-{digits}
      </label>
        ...
        <input
          ...
          onChange={(event) => {
            dispatch(
              updateLineParams({
                lineIndex,
                params: { [property]: +event.target.value }
              })
            )
          }}
        />
...
const LineEditorPanel = (props) => {
  ...
  const { color, totalD, input, params } = line
  ...
  return (
    <div className='panel'>
      <p className='panel-heading'>
        Line {lineIndex + 1} (D = {totalD.toFixed(3)}Gy)
      </p>
                ...
                <select
                  ...
                  onChange={(event) => {
                    dispatch(
                      updateLineParams({
                        lineIndex,
                        params: { model: event.target.value }
                      })
                    )
                  }}
                >
